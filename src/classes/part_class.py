...
class Part:
    ...
    def get_other_params(self, cursor=None, conn=None):
        sql = "SELECT machines_id FROM `production`.recipe r INNER JOIN `production`.machines_has_recipe mhr ON r.recipe_id = mhr.recipe_recipe_id INNER JOIN `production`.machines m ON mhr.machines_machines_id = m.machines_id WHERE recipe_id = {0}".format(
            self.recipe_id
        )
        cursor.execute(sql)
        res = cursor.fetchone()

        self.current_entity = int(res['machines_id'])
    ...
    # @functions.conn_decorator_method
    # def observe_next_entity(self, cursor=None, conn=None):
    #
    ...
    @functions.conn_decorator_method
    def update_attr(self, cursor=None, conn=None):
